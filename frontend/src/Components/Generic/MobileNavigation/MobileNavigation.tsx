...
const Navigation = ({ role }: INavigation) => {
  ...
  const navigationInstance =
    roleNotNull === "volunteer" ? navigationVolunteer : navigationManager;
      ...
      {navigationInstance.map((anchor) => {
        return (
          <Link
            key={anchor.path}
            to={anchor.path}
            className="flex flex-col items-center p-2 text-clr-primary hover:bg-hover hover:cursor-pointer hover:text-clr-primary"
          >
